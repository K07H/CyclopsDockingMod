...
public class CyclopsDockingModUI : MonoBehaviour
{
    ...
    private static float CDMScreenMaxPosX = Math.Max((float)Screen.width - CDMScreenWidth, 0f);

    private static float CDMScreenMaxPosY = Math.Max((float)Screen.height - CDMScreenHeight, 0f);

    public static Rect CDMScreen = new Rect(CDMScreenSttPosX, CDMScreenSttPosY, CDMScreenWidth, CDMScreenHeight);
    ...
    public static string CfgOpenUIKeyText = OpenUI.ToString();
    ...
    {
        if (_isToggled)
            UWE.Utils.lockCursor = _previousState;
        else
        {
            _previousState = UWE.Utils.lockCursor;
    ...
    private void CollapseWindow()
    {
        ...
        if (this.IsMinimized)
        {
            CDMScreen = new Rect(CDMScreenSttPosX, CDMScreenSttPosY, CDMScreenMinWidth, CDMScreenMinHeight);
            GUI.skin.window.alignment = TextAnchor.UpperLeft;
            GUI.skin.window.padding.left = 15;
            return;
        }
        CDMScreen = new Rect(CDMScreenSttPosX, CDMScreenSttPosY, CDMScreenWidth, CDMScreenHeight);
    ...
    {
        if (GUI.Button(new Rect(CDMScreen.width - 54f, 0f, 27f, 26f), this.IsMinimized ? "☐" : "-", this.IsMinimized ? StylesHelper._sBoldFontBtnStyle : GUI.skin.button))
            this.CollapseWindow();
        if (GUI.Button(new Rect(CDMScreen.width - 27f, 0f, 27f, 26f), "X", GUI.skin.button))
            this.CloseWindow();
    }
    ...
    {
        CDMScreenSttPosX = Math.Min(Math.Max(CDMScreen.x, 0f), CDMScreenMaxPosX);
        CDMScreenSttPosY = Math.Min(Math.Max(CDMScreen.y, 0f), CDMScreenMaxPosY);
        this.ScreenMenuBox();
        if (!this.IsMinimized)
            this.ModOptionsBox();
    ...
    {
        CDMScreen = GUI.Window(this.GetHashCode(), CDMScreen, new GUI.WindowFunction(this.DrawCDMScreen), "Cyclops Docking mod v2.0.7, by OSubMarin", GUI.skin.window);
    }
    ...
    private void OnGUI()
    {
        ...
        if (this._isOpenUIGrabKey && Event.current.isKey && Event.current.type == EventType.KeyDown)
        {
            ...
            OpenUI = Event.current.keyCode;
            CfgOpenUIKeyText = OpenUI.ToString();
            ConfigOptions.UpdateConfigFile(Environment.NewLine + "openModSettingsKey=" + Environment.NewLine, Environment.NewLine + "openModSettingsKey=" + CfgOpenUIKeyText + Environment.NewLine);
            ErrorMessage.AddDebug("Open mod settings key is now \"" + CfgOpenUIKeyText + "\".");
        }
        if (this._isManualDockingGrabKey && Event.current.isKey && Event.current.type == EventType.KeyDown)
        {
            ...
            CfgManualDockingKeyText = SubControlFixer.ManualDockingKey.ToString();
            ConfigOptions.UpdateConfigFile(Environment.NewLine + "manualDockingKey=" + Environment.NewLine, Environment.NewLine + "manualDockingKey=" + CfgManualDockingKeyText + Environment.NewLine);
            ErrorMessage.AddDebug("Manual docking key is now \"" + CfgManualDockingKeyText + "\".");
        }
        if (this.ShowUI)
            this.InitWindow();
    }
