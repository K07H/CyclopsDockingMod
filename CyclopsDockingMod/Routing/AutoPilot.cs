...
internal static class AutoPilot
{
    ...
    private static GameObject AddHudButton(GameObject original, Transform parent, Vector3 localPos, string objName, string tooltip, Sprite sprite, Sprite alternate, bool enableAnim = false, bool small = false)
    {
        ...
        GameObject gameObject = UnityEngine.Object.Instantiate(original, parent, true);
        gameObject.name = objName;
        gameObject.transform.localPosition = localPos;
        Image component2 = gameObject.GetComponent<Image>();
        if (component2 != null)
        {
            component2.sprite = _bg2;
            component2.overrideSprite = _bg1;
        }
        ...
        CyclopsEngineChangeState cyclopsEngineChangeState = gameObject.GetComponent<CyclopsEngineChangeState>();
        if (cyclopsEngineChangeState != null)
            cyclopsEngineChangeState.tooltipText = tooltip;
    ...
    private static void RestoreBtn(Transform engineOnUI, Transform centerHUD)
    {
        ...
        if (gameObject2 == null)
            return;
        AddHudButton(gameObject, engineOnUI, new Vector3(-20f, 760f, 0.004157424f), "CyclopsAutoPilotBtn", "CyclopsAutoPilotOn", _helm1, _helm2, true, false);
        AddHudButton(gameObject, engineOnUI, new Vector3(-200f, 760f, 0.004157424f), "CyclopsRouteSelectBtn", "CyclopsSelectRoute", _route2, _route1, false, false);
        AddHudLabel(gameObject2, engineOnUI, new Vector3(-1780f, -400f, 260f), gameObject.transform.eulerAngles, Lbl_NoRouteSelected);
        GameObject gameObject3 = AddHudButton(gameObject, engineOnUI, new Vector3(-230f, 900f, 0.004157424f), "CyclopsRouteEditBtn", "CyclopsEditRoute", _edit2, _edit1, false, true);
        GameObject gameObject4 = AddHudButton(gameObject, engineOnUI, new Vector3(-115f, 900f, 0.004157424f), "CyclopsRouteRemoveBtn", "CyclopsRemoveRoute", _remove2, _remove1, false, true);
        GameObject gameObject5 = AddHudButton(gameObject, engineOnUI, new Vector3(0f, 900f, 0.004157424f), "CyclopsConfirmRemoveBtn", "CyclopsConfirmRemove", _confirm2, _confirm1, false, true);
    ...
    internal static void RefreshHud(SubRoot sr, bool docked = true)
    {
        ...
        if (transform != null)
        {
            ...
            RestoreBtn(transform2, transform3);
            ShowHud(transform2, docked);
        }
    }
    ...
    {
        int nextRouteId = GetNextRouteId();
        string text = ((!string.IsNullOrWhiteSpace(name)) ? name : string.Format(Lbl_DefaultRouteName, (nextRouteId + 1).ToString()));
        Route route = new Route(nextRouteId, text, wayPoints, GetRouteLength(wayPoints), speed, basePartPosStt, basePartPosEnd);
        Routes.Add(route);
        return route;
    }
    ...
    {
        if (SubsPlayingRoutes.ContainsKey(pid))
            SubsPlayingRoutes[pid].SelectedRoute = rid;
        else
        {
            SubsPlayingRoutes.Add(pid, new SubRoutePlaying
            {
    ...
    internal static void StopPlayingRoute(string pid, Transform cyclopsHud = null, bool reachedEnd = true, bool endsOnNothing = false)
    {
        if (reachedEnd)
        {
            SelectRoute(pid, -1, cyclopsHud, Lbl_NoRouteSelected);
            if (endsOnNothing)
                ShowHud(cyclopsHud, false);
        }
        if (SubsPlayingRoutes.ContainsKey(pid))
        {
            SubsPlayingRoutes[pid].LastTime = -1f;
            SubsPlayingRoutes[pid].LastPos = null;
            SubsPlayingRoutes[pid].LastAngle = null;
            SubsPlayingRoutes[pid].IsStuck = false;
            SubsPlayingRoutes[pid].AlternateUnstuck = true;
            SubsPlayingRoutes[pid].AlternateUnstuckBis = false;
            SubsPlayingRoutes[pid].IsPlayingRoute = false;
        }
    }
    ...
    private static void StopSubPlayingRoute(string pid)
    {
        ...
        if (subRoot != null)
            StopPlayingRoute(pid, subRoot.transform.Find("HelmHUD/HelmHUDVisuals/Canvas_LeftHUD/EngineOnUI"), true, true);
    }

    internal static void StopSubsPlayingRoute(Vector3 pos, bool isRouteEndPoint)
    {
        foreach (KeyValuePair<string, SubRoutePlaying> keyValuePair in SubsPlayingRoutes)
        {
            if (isRouteEndPoint)
            {
                ...
                if (flag && keyValuePair.Value.Playing.BasePartPosEnd != null && FastHelper.IsNear(keyValuePair.Value.Playing.BasePartPosEnd.Value, pos) && keyValuePair.Value.IsPlayingRoute)
                    StopSubPlayingRoute(keyValuePair.Key);
            }
            else
            {
    ...
    internal static void StartPlayingRoute(string pid)
    {
        if (SubsPlayingRoutes.ContainsKey(pid))
        {
            int selectedRoute = SubsPlayingRoutes[pid].SelectedRoute;
            foreach (Route route in Routes)
            {
                if (route.Id == selectedRoute)
                {
                    SubsPlayingRoutes[pid].CurrentRouteIndex = 0;
                    SubsPlayingRoutes[pid].Playing = route;
                    SubsPlayingRoutes[pid].IsPlayingRoute = true;
                    break;
                }
            }
        }
    ...
    internal static void StartRecording(SubControl subCtrl, string routeName = null, int routeSpeed = 2, BasePart bp = null)
    {
        ...
        if (recordRouteController != null)
        {
            ErrorMessage.AddDebug(Lbl_RecordingNewRoute);
    ...
    {
        if (!IsRecording && !UndockStartRecord)
            return;
        ...
        if (component != null)
        {
            ...
            if (saveRoute)
            {
                Route route = AddRoute(component.RecordedWayPoints, component.RouteName, component.RouteSpeed, component.BasePartPosStt, component.BasePartPosEnd);
                ErrorMessage.AddDebug(string.Format(Lbl_RecordingStopped, route.Name));
            }
            else
                ErrorMessage.AddDebug(Lbl_RecordingCancelled);
            Transform transform = subCtrl.transform.Find("HelmHUD/HelmHUDVisuals/Canvas_LeftHUD/EngineOnUI");
            SelectRoute(subCtrl.GetComponent<PrefabIdentifier>().Id, -1, transform, Lbl_NoRouteSelected);
            if (bp == null)
                ShowHud(transform, false);
        }
    }
    ...
    internal static void LoadRoutes(string saveGame)
    {
        ...
        if (saveFolderPath != null)
        {
            ...
            if (File.Exists(text))
            {
                ...
                Logger.Log("INFO: Cyclops auto-pilot routes loaded. Player made {0} custom routes.", new object[] { Routes.Count });
                return;
            }
