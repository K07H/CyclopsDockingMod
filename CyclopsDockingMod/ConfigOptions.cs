...
internal class ConfigOptions
{
    ...
    private static void ProcessConfigLine(string str)
    {
        ...
        else if (str.StartsWith("noCyclopsText="))
        {
            ...
            {
                LblNoCyclopsDocked = str.Substring("noCyclopsText=".Length);
                Logger.Log("INFO: Loaded noCyclopsText: « " + LblNoCyclopsDocked + " »");
                return;
            }
        ...
        else if (str.StartsWith("cyclopsDockedText="))
        {
            ...
            {
                LblCyclopsDocked = str.Substring("cyclopsDockedText=".Length);
                Logger.Log("INFO: Loaded cyclopsDockedText: « " + LblCyclopsDocked + " »");
                return;
            }
        ...
        else if (str.StartsWith("climbIntoCyclopsText="))
        {
            ...
            {
                LblClimbInCyclops = str.Substring("climbIntoCyclopsText=".Length);
                Logger.Log("INFO: Loaded climbIntoCyclopsText: « " + LblClimbInCyclops + " »");
                return;
            }
        ...
        else if (str.StartsWith("basePieceRecipe="))
        {
            ...
            List<TechType> list = ParseIngredients(str.Substring("basePieceRecipe=".Length));
        ...
        else
        {
            ...
            {
                foreach (KeyValuePair<string, int> keyValuePair in autoPilotLang)
                {
    ...
    public static void LoadConfig()
    {
        ...
        {
            Logger.Log("WARNING: Cannot find configuration at \"" + ConfigFilePath + "\". Default settings will be used.");
            return;
        }
        string text = File.ReadAllText(ConfigFilePath, Encoding.UTF8);
        if (text == null)
        {
            Logger.Log("WARNING: Could not read configuration file at \"" + ConfigFilePath + "\". Default settings will be used.");
            return;
        }
        ...
        if (array != null)
        {
            ...
            string[] array2 = array;
            for (int i = 0; i < array2.Length; i++)
                ProcessConfigLine(array2[i]);
            return;
        }
        Logger.Log("WARNING: Configuration file at \"" + ConfigFilePath + "\" is empty. Default settings will be used.");
    }

    public static void UpdateConfigFile(string oldStr, string newStr)
    {
        if (!string.IsNullOrEmpty(oldStr) && !string.IsNullOrEmpty(newStr) && !string.IsNullOrEmpty(ConfigFilePath) && File.Exists(ConfigFilePath))
        {
            try
            {
                string text = File.ReadAllText(ConfigFilePath, Encoding.UTF8);
                if (!string.IsNullOrEmpty(text))
                {
                    ...
                    if (text2 != null)
                        File.WriteAllText(ConfigFilePath, text2, Encoding.UTF8);
                }
            }
            catch
            {
                string text5 = "An error happened while updating config file at \"" + ConfigFilePath + "\"!";
