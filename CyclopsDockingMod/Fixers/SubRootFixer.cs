...
public static class SubRootFixer
{
    ...
    {
        if (__instance.powerRelay != null && GameModeUtils.RequiresIngredients())
        {
            ...
            if (component != null && SubControlFixer.Docked(component.Id) && __instance.powerRelay.GetPower() < __instance.powerRelay.GetMaxPower())
            {
                ...
                float num;
                if (((root != null) ? root.powerRelay : null) != null && SubControlFixer.DockedSubs[component.Id].root.powerRelay.IsPowered() && SubControlFixer.DockedSubs[component.Id].root.powerRelay.ConsumeEnergy(CyclopsRechargeSpeed, out num) && num > 0f && num < 1f)
                {
