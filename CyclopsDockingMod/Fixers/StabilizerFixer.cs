...
public static class StabilizerFixer
{
    ...
    {
        List<string> list = SubsNotStabilizing.Keys.ToList<string>();
        if (list != null)
            foreach (string text in list)
                if (!string.IsNullOrEmpty(text))
                    SubsNotStabilizing[text] = ResetVal;
    }
    ...
    private static bool AdjustPosAndRot(SubRoot sub, Rigidbody rb, Vector3 pos, BasePart bp)
    {
        ...
        if (!FastHelper.IsNear(pos, vector))
        {
            ...
            float num2 = SpeedRatio(vector2.sqrMagnitude);
            vector2 = Vector3.ClampMagnitude(vector2, Mathf.Clamp01(0.1f * num2));
            vector2.y = AdjustVSpeed(num, vector2.y);
    ...
    public static void FixedUpdate_Postfix(Stabilizer __instance)
    {
        ...
        {
            Rigidbody rigidbody = __instance.body;
            ...
            {
                if (!SubsNotStabilizing.ContainsKey(component.Id))
                {
                    SubsNotStabilizing.Add(component.Id, ResetVal);
                }
                int key = SubsNotStabilizing[component.Id].Key;
                float value = SubsNotStabilizing[component.Id].Value;
                if (key < 1 || Time.time > value + 10f)
                {
                    Vector3? hatchPos = GetHatchPos(__instance.gameObject.transform);
                    if (hatchPos != null)
                    {
                        if (AdjustPosAndRot(__instance.subRoot, rigidbody, hatchPos.Value, SubControlFixer.DockedSubs[component.Id]))
                        {
                            SubsNotStabilizing[component.Id] = ResetVal;
                            return;
                        }
                        if (key < 100)
                        {
                            SubsNotStabilizing[component.Id] = new KeyValuePair<int, float>(key + 1, value);
                            return;
                        }
                        SubsNotStabilizing[component.Id] = new KeyValuePair<int, float>(key, Time.time);
                        return;
                    }
                }
            }
            ...
            {
                Vector3? hatchPos2 = GetHatchPos(__instance.gameObject.transform);
                if (hatchPos2 != null)
                    SubControlFixer.StabilizePlay(rigidbody, __instance.subRoot, component.Id, hatchPos2.Value, AutoPilot.SubsPlayingRoutes[component.Id]);
            }
        }
    }
}
